...
class DBConnection {
 public:
  DBConnection(const string& host, const string& dbname, const string& user,
    ...
    connection.reset(
        PQsetdbLogin(host.c_str(), to_string(port).c_str(), nullptr, nullptr,
                     dbname.c_str(), user.c_str(), password.c_str()),
        &PQfinish);

    if (PQstatus(connection.get()) != CONNECTION_OK &&
        PQsetnonblocking(connection.get(), 1) != 0) {
  ...
  shared_ptr<PGconn> get_connection() const { return connection; }
