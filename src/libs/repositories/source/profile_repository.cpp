...
void ProfileRepository::create(Profile &item, err_code &rc)
{
	...
	std::vector<std::vector<std::string>> query_result = {};
	std::string query = 
		(boost::format("insert into profile values(%1%, %2%, '%3%', '%4%', '%5%');")
		% id % user_id % username % avatar % birthday).str();
	if (auto ctrl = db_controller.lock())
	{
...
void ProfileRepository::update(Profile &item, err_code &rc)
{
	...
	std::vector<std::vector<std::string>> query_result = {};
	std::string query = 
		(boost::format("update profile set username = '%1%', avatar = '%2%', birthday = '%3%' where id = %4%;") 
		% username % avatar % birthday % id).str();
	if (auto ctrl = db_controller.lock())
	{
		if (ctrl->run_query(query, query_result))
			rc = OK;
