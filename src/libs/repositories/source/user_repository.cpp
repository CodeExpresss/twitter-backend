...
void UserRepository::update(User &item, err_code &rc) {
  ...
  std::vector<std::vector<std::string>> query_result = {};
  std::string query =
      (boost::format("update users set password = '%1%', email = '%2%', "
                     "session = %3%, is_active = %4% where id = %5%;") %
       password % email % session %
       boost::io::group(std::boolalpha, is_active) % id)
          .str();
  if (auto ctrl = db_controller.lock()) {
    if (ctrl->run_query(query, query_result))
      rc = OK;
...
void UserRepository::erase(int id, err_code &rc) {
  std::vector<std::vector<std::string>> query_result = {};
  std::string query =
      (boost::format("update users set is_active = false where id = %1%;") % id)
          .str();
  if (auto ctrl = db_controller.lock()) {
    if (ctrl->run_query(query, query_result))
      rc = OK;
